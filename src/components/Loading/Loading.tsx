...
const loadingStyles = tv({
	...
	variants: {
		type: {
			...
			component: 'animate-pulse duration-300 bg-slate-100/95',
			animation: 'scale-50'
		},
		show: {
			true: 'opacity-100',
...
export function Loading({ show = true, type = 'full', showLoad = false, ...rest }: LoadingProps) {
	...
	const load = <div className='border-slate-300 size-16 animate-spin rounded-full border-4 border-t-primary' />
